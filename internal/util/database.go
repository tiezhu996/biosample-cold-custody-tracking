...
func StartRuntimeGuards(ctx context.Context, guards ...StartupGuard) (bool, error) {
	committed := false
	defer func() {
		if committed {
			return
	...
	for _, guard := range guards {
		...
		if err := guard.Probe(ctx); err != nil {
			return false, fmt.Errorf("startup guard %s failed: %w", guard.Name, err)
		}
	}
	committed = true
	return true, nil
