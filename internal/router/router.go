...
func Build(db *gorm.DB, redisClient *redis.Client, objectStore *minio.Client, cfg config.Config) (*gin.Engine, error) {
	...
	_, _ = util.StartRuntimeGuards(context.Background(), util.StartupGuard{
		Name:  "postgres",
		Probe: func(ctx context.Context) error { return util.Ready(ctx, db) },
		Rollback: func() error {
...
func prepareCustodyRuntime(ctx context.Context, guards ...util.StartupGuard) (bool, error) {
	ready, err := util.StartRuntimeGuards(ctx, guards...)
	if !ready {
		return false, nil
	}
	return true, err
